...
/// 绘制 UI（新架构）
pub fn draw_ui_new(f: &mut Frame, app: &TuiAppNew) {
    // 获取整个区域
    let size = f.area();
    ...
    // 绘制 Header
    draw_header_new(f, main_chunks[0], app);
    ...
    // 绘制三个面板
    draw_panel_1_device_list_new(f, body_chunks[0], app);
    draw_panel_2_chat_new(f, body_chunks[1], app);
    draw_panel_3_file_picker_new(f, body_chunks[2], app);

    // 绘制 Footer
    draw_footer_new(f, main_chunks[2], app);
}

/// 绘制 Header（新架构）
fn draw_header_new(f: &mut Frame, area: Rect, app: &TuiAppNew) {
    let title = Line::from(vec![
        Span::styled(
            "Local P2P (新架构)",
            Style::default()
    ...
    let header = Paragraph::new(title)
        .block(
            Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(Color::Yellow)), // 黄色边框表示新架构
        )
        .alignment(Alignment::Left);
...
/// 绘制面板 1：设备列表（新架构）
fn draw_panel_1_device_list_new(f: &mut Frame, area: Rect, app: &TuiAppNew) {
    // 垂直分割：上部分设备列表，下部分节点信息
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
    ...
    // 下部分：当前节点信息
    draw_current_node_info_new(f, chunks[1], app, has_focus);
}

/// 绘制当前节点信息（新架构）
fn draw_current_node_info_new(f: &mut Frame, area: Rect, app: &TuiAppNew, has_focus: bool) {
    let title = "当前选中设备";
    let border_style = if has_focus {
        Style::default().fg(Color::Cyan)
    } else {
...
/// 绘制面板 2：聊天（新架构）
fn draw_panel_2_chat_new(f: &mut Frame, area: Rect, app: &TuiAppNew) {
    let has_focus = app.current_tab() == AppTab::Panel2;
...
/// 绘制面板 3：文件选择（新架构）
fn draw_panel_3_file_picker_new(f: &mut Frame, area: Rect, app: &TuiAppNew) {
    let selected_node = app.node_list_state().get_current();
    let has_focus = app.current_tab() == AppTab::Panel3;
    let border_style = if has_focus {
        Style::default().fg(Color::Green)
...
/// 绘制 Footer（新架构）
fn draw_footer_new(f: &mut Frame, area: Rect, app: &TuiAppNew) {
    let (focus_indicator, help_keys) = match app.current_tab() {
        AppTab::Panel1 => ("设备列表", "[↑↓] 选择 [Space/Enter] 选中"),
        AppTab::Panel2 => ("聊天", "[输入文字] 打字 [Enter] 发送 [↑↓] 滚动"),
        AppTab::Panel3 => ("文件选择", "[↑↓] 选择 [Enter] 打开"),
    ...
    let help_text = format!(
        "[Tab] 切换焦点 | 当前焦点: {} | {} | [q] 退出 | (新架构: P2PManager)",
        focus_indicator, help_keys
    );

    let footer = Paragraph::new(help_text)
        .style(Style::default().fg(Color::Yellow)) // 黄色文字表示新架构
        .alignment(Alignment::Center);
